...
select 'Tanisha Ethnic','918141152565','https://www.instagram.com/tanisha.ethnic/','Welcome to Tanisha Ethnic'
...
-- FINAL COUPON MANAGER MIGRATION (safe to re-run)
...
alter table public.coupons add column if not exists minimum_order numeric default 0;
...
-- Copy values from alternate legacy columns when present.
do $$ begin
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='coupons' and column_name='min_order') then
    execute 'update public.coupons set minimum_order=coalesce(minimum_order,min_order,0)';
  end if;
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='coupons' and column_name='max_discount') then
    execute 'update public.coupons set maximum_discount=coalesce(maximum_discount,max_discount)';
  end if;
end $$;
alter table public.coupons drop constraint if exists coupons_discount_type_check;
update public.coupons set discount_type=case
  when lower(coalesce(discount_type,'')) in ('percent','percentage','%') then 'percent'
  when lower(coalesce(discount_type,'')) in ('fixed','flat','amount','fixed_amount') then 'fixed'
  when lower(coalesce(discount_type,'')) in ('free_shipping','freeship','shipping') then 'free_shipping'
  else 'percent' end;
...
alter table public.coupons add constraint coupons_discount_type_check check(discount_type in ('percent','fixed','free_shipping'));
...
insert into public.coupons(code,description,discount_type,discount_value,minimum_order,maximum_discount,active)
